...
cb&&cb(response)},function(response){cb&&cb(response)})},this.subscribeToClub=function(plan,cb){Request.post("/api/freshclub/subscribeToClub",{plan:plan}).then(function(response){cb&&cb(response)},function(response){cb&&cb(response)})},this.subscribeToClubV2=function(reqBody,cb){Request.post("/api/freshclub/subscribeToClubV2",reqBody).then(function(response){cb&&cb(response.data)},function(response){cb&&cb(response)})},this.updateGenCart=function(genCart,cb){Request.post("/api/payment/updateGenCart",{cart:genCart}).then(function(response){cb&&cb(response)},function(response){cb&&cb(response)})},this.getLazyPayApproval=function(cb){Request.post("/api/cart/lazypay/getEligibility").then(function(response){cb&&cb(response)},function(response){cb&&cb(response)})},this.fmVerifyLazypayOtp=function(fmReqObj,callback){Request.get("/api/payment/lazyPay/"+fmReqObj.orderId+"/"+fmReqObj.otp).then(function(response){callback&&callback(response.data)})},this.validateVpa=function(fmReqObj,callback){Request.get("/api/payment/upi/validateVPA/"+fmReqObj.vpa).then(function(response){callback&&callback(response.data)})},this.tezCheckStatusForTransaction=function(fmReqObj,cb){Request.post("/api/payment/upi/checkStatusForTransaction",fmReqObj).then(function(response){cb&&cb(response)},function(response){cb&&cb(response)})},this.tezPhNoCheckStatusForTrans=function(fmReqObj,cb){Request.post("/api/payment/google-tez/checkStatusForTransaction",fmReqObj).then(function(response){cb&&cb(response)},function(response){cb&&cb(response)})},this.markAsUndelivered=function(orderId,callback){Request.get("/api/orders/markAsUndelivered/"+orderId).then(function(response){callback&&callback(response.data)})},this.fmChangeAddress=function(data,cb){Request.post("/api/orders/changeAddress",data).then(function(res){cb&&cb(res)},function(res){cb&&cb(res)})},this.fmRedeemGiftCard=function(data,cb){Request.post("/api/freshmoney/giftCard",data).then(function(res){cb&&cb(res)},function(res){cb&&cb(res)})},this.riskContext=function(data){Request.get("/api/payment/riskContext/"+data.fm_uuid)},this.triggerTradeTracker=function(data){Request.post("/api/orders/triggerTradeTracker",data).then(function(res){},function(res){})}}]),app.controller("MainBulkOrder",["$scope","$timeout","$sce","Api","$localstorage","$window","SimplPaymentService","TrackingService",function($scope,$timeout,$sce,Api,$localstorage,$window,SimplPaymentService,TrackingService){$scope.fmShowElems=window.fmShowElems,$scope.contactInfo=window.contactInfo,$scope.forms={bulkOrder:{}},$scope.forms.bulkOrder.query="",$scope.forms.bulkOrder.email="",$scope.forms.bulkOrder.errorMessage=null,$scope.bulkOrderError={},$scope.scriptsCheck=function(s){var div=document.createElement("div");div.innerHTML=s;var scripts=div.getElementsByTagName("script"),i=scripts.length;return!!i},$scope.fmHandlebulkOrderSubmit=function(){if($scope.bulkOrderError={},null==$scope.forms.bulkOrder.email||""==$scope.forms.bulkOrder.email)return void($scope.forms.bulkOrder.errorMessage="Please enter email");if(!$scope.forms.bulkOrder.email.match(/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/))return $scope.bulkOrderError.emailInvalid=!0,void($scope.forms.bulkOrder.errorMessage="Please enter valid email");if(null==$scope.forms.bulkOrder.name||""==$scope.forms.bulkOrder.name||$scope.scriptsCheck($scope.forms.bulkOrder.name))return $scope.forms.bulkOrder.errorMessage="Please enter valid name",void($scope.bulkOrderError.name=!0);if(null==$scope.forms.bulkOrder.phoneNumber||""==$scope.forms.bulkOrder.phoneNumber)return $scope.bulkOrderError.phoneNumberEmpty=!0,void($scope.forms.bulkOrder.errorMessage="Please enter phone number");if(!$scope.forms.bulkOrder.phoneNumber.match(/^\d{10}$/))return $scope.forms.bulkOrder.errorMessage="Please enter valid phone number",void($scope.bulkOrderError.phoneNumberInvalid=!0);if(null==$scope.forms.bulkOrder.query||""==$scope.forms.bulkOrder.query)return $scope.forms.bulkOrder.errorMessage="Please enter your query",void($scope.bulkOrderError.query=!0);if(!_.keys($scope.bulkOrderError).length){var bulkOrderObject={mobileNumber:$scope.forms.bulkOrder.phoneNumber,message:$scope.forms.bulkOrder.query,name:$scope.forms.bulkOrder.name};$scope.forms.bulkOrder.email&&$scope.forms.bulkOrder.email.length>0&&(bulkOrderObject.email=$scope.forms.bulkOrder.email),$scope.bulkOrderLoading=!0,TrackingService.postEvent("Clicked",{action:"submit",screenName:"/bulk-order"}),Api.handlebulkOrderSubmit(bulkOrderObject,function(response){$scope.forms.bulkOrder.errorMessage=null,200==response.status?(TrackingService.postEvent("bulkOrder company sign up",bulkOrderObject),$scope.forms.bulkOrder.successMessage="Thank you for your interest in FreshMenu BulkOrder Program. We will get back to you very soon."):$scope.forms.bulkOrder.errorMessage="Oops, either there is internet connectivity issue or our server is down, please try again!",$scope.bulkOrderLoading=!1})}},$scope.fmSubscribeEmail=null,$scope.fmSubscribeEmailError=!1,$scope.fmSubscribeError=!1,$scope.fmSubscribeSuccess=!1,$scope.validateSubscribeEmail=function(){$scope.fmSubscribeError=!1,$scope.fmSubscribeSuccess=!1,$scope.fmSubscribeEmail&&$scope.fmSubscribeEmail.length>0&&/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/.test($scope.fmSubscribeEmail)?$scope.fmSubscribeEmailError=!1:$scope.fmSubscribeEmailError=!0},$scope.fmSubscribeToEmail=function(){$scope.validateSubscribeEmail(),$scope.fmSubscribeEmail&&$scope.fmSubscribeEmail.length>0&&!$scope.fmSubscribeEmailError&&Api.fmSubscribeToEmail($scope.fmSubscribeEmail,function(data){data.data?$scope.fmSubscribeSuccess=!0:$scope.fmSubscribeError=!0})}}]),app.controller("CommonHeader",["$scope","$timeout","$sce","$window","Api","TrackingService",function($scope,$timeout,$sce,$window,Api,TrackingService){$scope.currentPathname=window.location.pathname.split("/"),$scope.currentPathname.length&&$scope.currentPathname.length>2&&($scope.currentPathname[1]=$scope.currentPathname[2]),$scope.fmShowElems=window.fmShowElems}]);